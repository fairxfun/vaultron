...
pub fn generate_mnemonic(word_count: usize) -> Result<String, EnclaveCryptoError> {
    let mnemonic = Mnemonic::generate(word_count).map_err(|_| EnclaveCryptoError::MnemonicGenerationError)?;
...
pub fn mnemonic_to_seed(mnemonic: &str, passphrase: &str) -> Result<Vec<u8>, EnclaveCryptoError> {
    ...
    match pbkdf2::<Hmac<Sha512>>(mnemonic.as_bytes(), salt.as_bytes(), 2048, &mut seed) {
        Ok(_) => Ok(seed),
        Err(_) => Err(EnclaveCryptoError::MnemonicToSeedError),
    }
}

pub fn derive_key_from_path(seed: &[u8], path: &str) -> Result<Vec<u8>, EnclaveCryptoError> {
    let xprv = XPrv::new(seed).map_err(|_| EnclaveCryptoError::DeriveKeyFromPathError)?;
    let derivation_path = DerivationPath::from_str(path).map_err(|_| EnclaveCryptoError::DeriveKeyFromPathError)?;
